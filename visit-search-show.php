...
<div class="content-wrapper clearfix">
	...
	<table width="580" align="center" cellpadding="3" cellspacing="0" border="1">
		<tr align=center><td width="200">對象姓名</td><td width = "200">電話</td><td width="200">探訪者</td><td width = "200">日期</td><td width = "300">情&nbsp形</td><td width = "300">備&nbsp註</td></tr>
		<?php
			if($name!=null && $date==null)
			{
				$sql =<<<EOF
				SELECT name,phone,visitor,date,situation,remark from namelist as a inner join visitinfo as b on a.id=b.nameid where name LIKE '$name%' order by date;
EOF;
				$ret = pg_query($db, $sql);
				if(!$ret){
				...
				while($row = pg_fetch_row($ret)){
					echo "<tr align=center><td valign=\"top\" align=\"left\">". $row[0] . "</td><td>".$row[1]."</td><td>".$row[2]."</td><td>".$row[3]."</td><td>".$row[4]."</td><td>".$row[5]."</td></tr>";
					}
				pg_close($db);
			}
...
EOF;
			...
			{
				$sql =<<<EOF
				SELECT name,phone,visitor,date,situation,remark from namelist as a inner join visitinfo as b on a.id=b.nameid where name LIKE '$name%', date LIKE '$date%' order by date;
EOF;
				$ret = pg_query($db, $sql);
				if(!$ret){
				echo pg_last_error($db);
