...
{
	echo "<script>if(confirm('請確實填寫看望者姓名、日期、被看望者姓名與情形')){document.location.href='U&D-record.php?id=$nameid&visitor=$visitor&date=$date&situation=$situation'};</script>";
	return 0;
}
else
{
	//update需有目標(原始的name,date跟visitor)
	 $sql =<<<EOF
      update visitinfo set nameid=$nameid,visitor='$visitor',date='$date',situation='$situation' where nameid=$n and visitor='$v' and date='$d';
EOF;
$ret = pg_query($db, $sql);
pg_close($db);
echo "更新紀錄成功!";
